...
D = Path(r"C:\00\Projects\LearningHub\_campaign\omul_la_calculator_2026_09_13\S_reparatii\cls5\lectia5-reguli\diacritice")
...
for inp in sorted(D.glob("in_*.txt")):
    ...
    if spec.exists():
        for raw in spec.read_bytes().decode("utf-8").split("\n"):
            ...
            new = ind + txt
            if len(new) != len(old) or any(o != n and not (o == n.translate(HARTA) and o != n) for o, n in zip(old, new)):
                bad += 1
