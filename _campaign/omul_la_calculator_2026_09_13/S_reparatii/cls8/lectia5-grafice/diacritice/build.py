...
D = Path(r"C:\00\Projects\LearningHub\_campaign\omul_la_calculator_2026_09_13\S_reparatii\cls8\lectia5-grafice\diacritice")
...
for inp in sorted(D.glob("in_*.txt")):
    ...
    if spec.exists():
        for raw in spec.read_bytes().decode("utf-8").split("\n"):
            ...
            while raw[k].isdigit():
                k += 1
            nr, sep, txt = raw[:k], raw[k], raw[k + 1:]
            i = int(nr) - 1 - base
            old = lines[i]
            body = old.rstrip("\r\n")
            end = old[len(body):]
            ...
            if sep == "|":
                new = ind + txt
            else:  # '~' : inlocuiri 'vechi=>nou;;vechi=>nou' (toate aparitiile din linie)
                new = body
                for pair in txt.split(";;"):
                    a, b = pair.split("=>", 1)
                    if a not in new:
                        print(f"[{nn}:{nr}] lipseste fragmentul {a!r}")
                        bad += 1
                    new = new.replace(a, b)
