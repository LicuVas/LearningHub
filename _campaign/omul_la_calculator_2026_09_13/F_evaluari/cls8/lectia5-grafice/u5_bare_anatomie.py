...
getBoundingClientRect pentru .chart-bar, .chart-bar-group si .chart-visual. Iesire: u5_bare_anatomie.txt (<= 20 randuri tiparite)."""
import asyncio
import functools
import http.server
import json
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(f"http://127.0.0.1:8765/{REL}")
        ...
        await pg.locator("#atom-6 .chart-visual").screenshot(path=str(L / "u5_bare_anatomie.png"))
        # captura vizuala = pas_06_atom_intreg.png (H_vede, dupa parcurgere)
        await br.close()
    srv.shutdown()
    txt = json.dumps(res, ensure_ascii=False, indent=1)
    (L / "u5_bare_anatomie.txt").write_text(txt, encoding="utf-8")
    print(txt[:1500])
