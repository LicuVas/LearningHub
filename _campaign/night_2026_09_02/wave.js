export const meta = {
  name: 'lh-night-wave',
  description: 'Construieste lectii Format C (Guided Atomic) pentru clasele reale de liceu/maistri/postliceal ale prof. Gurlan, dintr-un lot primit prin args. Scaffold -> Build -> Verify -> Fix. Sonnet la executie, Opus la verificare.',
  phases: [
...
// ── args: { label, audience, flavor, modules:[{cls,module,title,icon,desc,indexPath,lessons:[...]}] } ──
let A = args
if (typeof A === 'string') { try { A = JSON.parse(A) } catch (e) { A = {} } }
if (!A || typeof A !== 'object') A = {}
const LABEL = A.label || 'lot'
const AUDIENCE = A.audience || 'elevi de liceu'
const FLAVOR = A.flavor || 'exemple potrivite specializarii'
const MODULES = A.modules || []
if (!MODULES.length) { return { error: 'args.modules gol', argsType: typeof args } }
